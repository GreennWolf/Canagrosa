...
const SampleList = () => {
  ...
  const loadCatalogs = useCallback(async () => {
    try {
      // En lugar de usar sampleService.getRelatedCatalogs, ahora llamamos 
      // a cada servicio por separado
      const [clientesData, tiposMuestraData, tiposAnalisisData] = await Promise.all([
        clientesService.obtenerParaCombo(),
        tiposMuestraService.obtenerTodos(),
        tiposAnalisisService.obtenerTodos()
      ]);
      
      // Transformar datos para los dropdowns
      const clientOptions = clientesData.map(client => ({
        value: client.ID_CLIENTE,
        label: client.NOMBRE
      }));
      
      const sampleTypeOptions = tiposMuestraData.map(type => ({
        value: type.ID_TIPO_MUESTRA,
        label: type.NOMBRE
      }));
      
      const analysisTypeOptions = tiposAnalisisData.map(type => ({
