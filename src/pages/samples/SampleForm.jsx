...
const SampleForm = () => {
  ...
  useEffect(() => {
    const fetchCatalogs = async () => {
      ...
      try {
        ...
        // Cargar baños
        const bathsResponse = await api.catalogs.getBaths();
        setBaths(
          bathsResponse.data.map((bath) => ({
            value: bath.ID_BANO,
            label: bath.NOMBRE
          }))
        );
        ...
        // Cargar centros
        const centersResponse = await api.catalogs.getCenters();
        setCenters(
        ...
        const samplingEntitiesResponse = await api.catalogs.getSamplingEntities();
        setSamplingEntities(
          samplingEntitiesResponse.data.map((entity) => ({
        ...
        const deliveryEntitiesResponse = await api.catalogs.getDeliveryEntities();
        setDeliveryEntities(
          deliveryEntitiesResponse.data.map((entity) => ({
        ...
        // Cargar formatos
        const formatsResponse = await api.catalogs.getFormats();
        setFormats(
        ...
        // Cargar usuarios
        const usersResponse = await api.catalogs.getUsers();
        setUsers(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.samples.create(formData);
